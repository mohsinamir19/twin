...
terraform init -input=false \
  -backend-config="bucket=stwin-terraform-state-${AWS_ACCOUNT_ID}" \
...
if ($Environment -eq "prod") {
    ...
    terraform apply -var-file=prod.tfvars -var-file="secrets.tfvars" -var="project_name=$ProjectName" -var="environment=$Environment" -auto-approve
} else {
    # Added -var-file="secrets.tfvars"
    terraform apply -var-file="secrets.tfvars" -var="project_name=$ProjectName" -var="environment=$Environment" -auto-approve
}
